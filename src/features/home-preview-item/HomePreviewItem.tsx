...
import { setIsRegistrationModalOpen } from '@/views/store/modals.slice'
...
export const HomePreviewItem = (props: IPreview) => {
  ...
  const isOpen = useSelector((state: RootState) => state.modals.isRegistrationModalOpen)
  ...
  return (
    <div>
      <RegistrationModal
        isOpen={isOpen}
        setIsOpen={v => {
          dispatch(setIsRegistrationModalOpen(v))
        }}
      />
      <div className="overflow-hidden lg:rounded-[35px] md:rounded-[20px] rounded-[13px] relative">
        <ImageWithSkeleton
          ...
          width={1414}
          height={876}
          className="w-full md:h-full h-434px object-cover object-center"
        />
          ...
          .map((catalog, index) => (
            <SwiperSlide key={index} className="cursor-pointer">
              <div className="w-full flex flex-col lg:pt-[16px] pt-[10px]">
                <div
                  className={`w-full overflow-hidden lg:rounded-[42px] rounded-[25px] ${
                ...
                >
                  <ImageWithSkeleton
                    ...
                    width={334}
                    height={565}
                    className="w-full object-center object-cover 2xl:h-[564px] lg:h-[350px] md:h-[250px] h-348px"
                  />
                </div>
                <div className="lg:mt-[34px] mt-[15px] lg:text-[20px] md:text-[14px] text-[4.53vw] font-medium text-center">
                  Loremi Ipsumi
                </div>
              </div>
            </SwiperSlide>
      ...
      <div className="md:hidden block w-118px m-[10.14vw_auto_0px] h-2px bg-[#D9D9D9] rounded-[2px]">
        <div
          style={{ width: `${(100 / (catalogs.length - 1)) * (activeSlide + 1)}%` }}
          className="bg-purple h-full rounded-[2px] transition-all"
        ></div>
      </div>
    </div>
  )
