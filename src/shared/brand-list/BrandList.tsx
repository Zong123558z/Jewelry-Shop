...
import { toggleBrandItem } from '@/views/store/searchFilter.slice'
...
export const BrandList = () => {
  ...
  const handleClick = (value: string) => {
    dispatch(toggleBrandItem(value))
  }
  ...
  return (
    <div className="w-full lg:text-[16px] md:text-[14px] text-[4.27vw]">
      ...
      {isOpen && (
        <ul
          className={`w-full md:mb-[0px] mb-[15px] lg:pl-[20px] md:pl-[0px] pl-[2.67vw] md:pr-0 pr-[2.67vw] grid grid-cols-2 lg:gap-[13px_9px] md:gap-[9px] gap-[2.6vw] lg:mt-[20px] md:mt-[10px] mt-[2.67vw] transition-all`}
        >
          {brands.map(brand => (
            <li
              onClick={() => handleClick(brand.brandValue)}
              key={brand.brandValue}
              className={`border cursor-pointer transition-all active:scale-95 ${
                brand.active ? 'border-purple' : 'border-transparent'
              } w-full md:h-[49px] h-49px rounded-[7px] bg-white flex items-center justify-center md:p-[4.5px_45px] p-[1.2vw_10vw]`}
            >
              <Image
                src={brand.photo}
                alt="brand-img"
