...
const Flag = React.lazy(() =>
  import('react-world-flags').then(module => ({
    ...
    <div className="relative select-none">
      <div className="flex items-center md:gap-[8px] gap-[1.34vw]">
        <Suspense fallback={null}>
          <Flag
            code={flagList[activeIndex].flagCode}
            style={{ width: isMobileDevice ? '5.87vw' : 34, height: isMobileDevice ? '3.73vw' : 22 }}
          />
        </Suspense>

        <button type="button" onClick={() => setIsOpen(!isOpen)}>
          <svg
            className={`transition-all md:w-[18px] md:h-[10px] w-[3vw] h-[2vw] ${isOpen ? 'rotate-[180deg]' : ''}`}
            width="18"
            height="10"
            viewBox="0 0 18 10"
            fill="none"
          ...
          >
            <path
              d="M1.13354 0.926086L8.35258 8.14512C8.7431 8.53564 9.37626 8.53564 9.76679 8.14512L16.9858 0.926086"
              stroke="#2D2B2D"
              strokeWidth="1.5"
              strokeLinecap="round"
      ...
      <ul
        className={`absolute z-[50] overflow-hidden overflow-y-scroll flex flex-col md:gap-[5px] gap-[1.5vw] top-[130%] transition-all md:w-[110px] w-75px md:h-[150px] h-100px ${
          isOpen ? '' : 'pointer-events-none opacity-0 translate-y-[-10px]'
        } bg-white border border-[#ECECEC] rounded-[9px]`}
      >
        {flagList.map((flag, index) => (
          <li
            onClick={() => handleClick(index)}
            key={index}
            className={`cursor-pointer flex items-center p-[5px_10px_5px_5px] justify-between ${
              index === activeIndex ? 'bg-gray' : ''
            }`}
          >
            <Flag
              code={flag.flagCode}
